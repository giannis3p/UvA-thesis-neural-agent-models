...
input_layer = Input(shape=(sequence_length, 500, 500, 6))
...
model = Sequential([
    ...
    LSTM(units=64),
    Dense(units=500*500*6, activation='relu'),
    Reshape((500, 500, 6))
])

model.compile(optimizer=optimizer, loss='mse',  metrics=[r_squared, 'mape', accuracy, average_relative_rmse, 'msle', 'mae'])

history = model.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=500, batch_size=8, callbacks=[early_stopping, lr_scheduler_callback])
...
df_all_timesteps.to_csv('C-LSTM(500x500)(82-100hrs).csv', index=False)
...
output_dir = 'plots(500x500)/plots-C-LSTM'
...
csv_filename = 'c_500_82_loss_data.csv'

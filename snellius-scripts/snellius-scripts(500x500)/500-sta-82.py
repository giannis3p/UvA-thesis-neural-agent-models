...
sorted_concatenated_csv = "final run concat 500x500.csv"
...
class STALSTM(tf.keras.Model):
    def __init__(self, hidden_size, input_shape):
        ...
        self.fc3 = Dense(np.prod(input_shape[1:]), activation='relu')  # adjust output size
        self.reshape = Reshape(input_shape[1:])  # reshape to match output shape

    def call(self, input_data):
        lstm_output = self.lstm(input_data)
        attended_output = self.attention(lstm_output, input_data)
        batch_size = tf.shape(attended_output)[0]
        attended_output_flat = tf.reshape(attended_output, (batch_size, -1))  # flatten for fully connected layers
        x = self.fc1(attended_output_flat)
        x = self.fc2(x)
        output = self.fc3(x)
...
sample_input = tf.convert_to_tensor(X_train[:1], dtype=tf.float64)  # take a sample batch
...
df_all_timesteps.to_csv('STA-LSTM(500x500)(82-100hrs).csv', index=False)
...
output_dir = 'plots(500x500)/plots-STA-LSTM'
...
csv_filename = 'sta_500_82_loss_data.csv'
